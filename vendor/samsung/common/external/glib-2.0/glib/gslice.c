                              ...
                              gsize      count) /* must be >= MIN_MAGAZINE_SIZE */
{
  ...
  magazine_chain_count (current) = (gpointer)(intptr_t) count;
  /* stamp magazine */
  magazine_cache_update_stamp();
                             ...
                             gsize *countp)
{
  ...
  if (!allocator->magazines[ix])
  ...
  else
    {
      ...
      *countp = (gsize)(intptr_t) magazine_chain_count (current);
                    ...
                    gsize      chunk_size)
{
  ...
  /* mask page adress */
  addr = ((gsize)(intptr_t) mem / page_size) * page_size;
  /* assert alignment */
  mem_assert (aligned_memory == (gpointer)(intptr_t) addr);
                           ...
                           gpointer mem)
{
  ...
  guint ix = SLAB_INDEX (allocator, chunk_size);
  gsize page_size = allocator_aligned_page_size (allocator, SLAB_BPAGE_SIZE (allocator, chunk_size));
  gsize addr = ((gsize)(intptr_t) mem / page_size) * page_size;
  /* mask page adress */
  guint8 *page = (guint8*)(intptr_t) addr;
